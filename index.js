...
const phoneUtil = (
  libPhoneNumber.PhoneNumberUtil || libPhoneNumber.default?.PhoneNumberUtil
).getInstance();
...
watchFile(CONFIG_PATH, async () => {
  try {
    ...
    const prefStr =
      Array.isArray(global.prefixes) && global.prefixes.length
        ? global.prefixes.join(" ")
        : "-";
    const ownersStr =
      Array.isArray(global.owner) && global.owner.length
        ? global.owner
            .map((o) => (Array.isArray(o) ? o[0] || "" : o || ""))
            .filter(Boolean)
            .join(", ")
        : "-";
    const cfgInfo = `\n╭─────────────────────────────◉\n│ ${chalk.black.bgRedBright.bold(
      "        🔁 CONFIG ACTUALIZADA        "
    )}\n│ 「 🗂 」${chalk.cyan("Archivo: config.js")}\n│ 「 🧩 」${chalk.yellow(
      "Prefijos: "
    )}${chalk.white(prefStr)}\n│ 「 👑 」${chalk.yellow(
      "Owners:   "
    )}${chalk.white(ownersStr)}\n╰─────────────────────────────◉\n`;
    console.log(cfgInfo);
  } catch (e) {
...
async function loadPlugins() {
  ...
  try {
    ...
    const plugInfo = `\n╭─────────────────────────────◉\n│ ${chalk.red.bgBlueBright.bold(
      "        🧩 PLUGINS CARGADOS        "
    )}\n│ 「 📦 」${chalk.yellow("Total: ")}${chalk.white(
      total
    )}\n╰─────────────────────────────◉\n`;
...
async function importAndIndexPlugin(fullPath) {
  try {
    ...
    if (Array.isArray(plug.command))
      plug.command = plug.command.map((c) =>
    ...
    else if (Array.isArray(plug.command))
      cmds.push(...plug.command.filter((c) => typeof c === "string"));
    for (const c of cmds) {
    ...
    try {
      ...
      const errBox = `\n╭─────────────────────────────◉\n│ ${chalk.white.bgRed.bold(
        "        ❌ PLUGIN LOAD ERROR        "
      )}\n│ 「 🧩 」${chalk.yellow("Plugin: ")}${chalk.white(
        fname
      )}\n│ 「 ⚠️ 」${chalk.yellow("Error:  ")}${chalk.white(
        e.message || e
      )}\n╰─────────────────────────────◉\n`;
...
try {
  const dbInfo = `\n╭─────────────────────────────◉\n│ ${chalk.red.bgBlueBright.bold(
    "        📦 BASE DE DATOS        "
  )}\n│ 「 🗃 」${chalk.yellow("Archivo: ")}${chalk.white(
    DB_PATH
  )}\n╰─────────────────────────────◉\n`;
  console.log(dbInfo);
} catch {}
...
try {
  ...
  const info = `\n╭─────────────────────────────◉\n│ ${chalk.red.bgBlueBright.bold(
    ...
    `Autor: 𝙷𝙴𝚁𝙽𝙰𝙽𝙳𝙴𝚉 (@${config.ownerNumber || "50496926150"})`
  )}\n│「 👑 」${chalk.cyan(
    "𝙲𝚁𝙴𝙰𝙳𝙾𝚁 𝚄𝙽𝙸𝙲𝙾: 𝙷𝙴𝚁𝙽𝙰𝙽𝙳𝙴𝚉"
  )}\n╰─────────────────────────────◉\n\n╭─────────────────────────────◉\n│ ${chalk.red.bgMagenta.bold(
    "        ⏰ HORA ACTUAL        "
  )}\n│「 🕒 」${chalk.magenta(
    `${currentTime}`
  )}\n╰─────────────────────────────◉\n`;
...
async function startBot() {
  ...
  async function generatePairingCodeWithRetry(number, maxAttempts = 5) {
    ...
    while (attempt < maxAttempts) {
      try {
        ...
        const status =
          err?.output?.statusCode || err?.output?.payload?.statusCode;
        const transient =
          status === 428 ||
          err?.code === "ENOENT" ||
  ...
  async function maybeStartPairingFlow() {
    ...
    async function promptForNumber(initialMsg) {
      ...
      while (attempts < 5 && !obtained) {
        ...
        console.log(
          chalk.yellow(
            `[Pairing] Número no válido: ${cleaned}. Intenta de nuevo.`
          )
        );
        attempts++;
      }
    ...
    async function persistBotNumberIfNeeded(num) {
      try {
        ...
        console.log(
          chalk.red("[Config] No se pudo actualizar botNumber:", e.message)
        );
      }
    }
    ...
    const candidate =
      (config.botNumber ? config.botNumber.toString() : "")
        .trim()
        .replace(/[^0-9]/g, "") || primaryOwnerNumber().replace(/[^0-9]/g, "");
    if (candidate) {
      let confirm = await ask(
        `\n${chalk.cyan("Detectado número configurado:")} ${chalk.yellow(
          "+" + candidate
        )} ${chalk.white("¿Usar este número? (si/no): ")}`
      );
    ...
    if (!number) {
      number = await promptForNumber(
        `\n╭─────────────────────────────◉\n│ ${chalk.black.bgGreenBright.bold(
          "  📞 INGRESO DE NÚMERO WHATSAPP  "
        )}\n│「 ✨ 」${chalk.whiteBright(
          "Introduce tu número con prefijo de país"
        )}\n│「 🔃 」${chalk.yellowBright(
          "Ejemplo: +57321XXXXXXX"
        )}\n│\n│${chalk.gray(
          "Puede incluir +, se ignorarán espacios."
        )}\n╰─────────────────────────────◉\n${chalk.magenta(
          "--->"
        )} ${chalk.bold("Número: ")}`
      );
      if (!number) {
        console.log(
          chalk.red(
            "[Pairing] No se obtuvo un número válido. Reinicia con --code."
          )
        );
      ...
      !config.botNumber ||
      config.botNumber.replace(/[^0-9]/g, "") !== number
    ) {
      await persistBotNumberIfNeeded(number);
    }
    ...
    const launchCodeGeneration = async () => {
      ...
      try {
        console.log(
          chalk.gray(`[Pairing] Generando código para +${number} ...`)
        );
        ...
        console.log(
          `\n╭─────────────────────────────◉\n│ ${chalk.black.bgMagentaBright.bold(
            "🔐 CÓDIGO DE VINCULACIÓN"
          )}\n│「  」${chalk.bold.red(formatted)}   ${chalk.gray(
            `(${ms} ms)`
          )}\n│「  」${chalk.whiteBright(
  ...
  sock.ev.on("connection.update", async (update) => {
    ...
    if (qr && method === "qr") {
      console.clear();
      console.log(
        chalk.cyan("Escanea este QR con WhatsApp (Dispositivos vinculados):")
      );
      try {
      ...
      console.log(
        chalk.gray("Para usar código de emparejamiento: reinicia con --code")
      );
    }
    if (
      method === "code" &&
    ...
    if (connection === "close") {
      ...
      if (statusCode !== DisconnectReason.loggedOut) {
        console.log("𝙲𝙾𝙽𝙴𝙲𝚃𝙰𝙽𝙳𝙾 𝙺𝙰𝚁𝙱𝙾𝚃....");
        startBot();
      } else {
        console.log(
          "[𝚂𝙴𝚂𝙸Ó𝙽 𝙲𝙴𝚁𝚁𝙰𝙳𝙰] 𝙱𝚘𝚛𝚛𝚊 𝚕𝚊 𝚌𝚊𝚛𝚙𝚎𝚝𝚊 𝚍𝚎 𝚌𝚛𝚎𝚍𝚎𝚗𝚌𝚒𝚊𝚕𝚎𝚜 𝚢 𝚟𝚞𝚎𝚕𝚟𝚎 𝚊 𝚟𝚒𝚗𝚌𝚞𝚕𝚊𝚛."
        );
      }
      ...
      try {
        ...
        const userName =
          sock?.user?.name || sock?.user?.verifiedName || "Desconocido";
        console.log(chalk.green.bold(`[ ✅️ ]  𝙲𝙾𝙽𝙴𝙲𝚃𝙰𝙳𝙾 𝙰: ${userName}`));
        const jid = rawId;
        ...
        if (num && !config.botNumber && !global.botNumber) {
          try {
            ...
            if (updated !== file) {
              ...
              global.botNumber = num;
              console.log(
                chalk.gray("[Config] botNumber autocompletado en config.js")
              );
            }
          } catch (e) {
            console.log(
              chalk.red("[Config] Error guardando botNumber auto:", e.message)
            );
          }
        }
      } catch (e) {
